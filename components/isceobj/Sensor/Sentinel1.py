...
class Sentinel1(Component):
	...
	def validateUserInputs(self):
		...
		swathid = 's1?-iw-grd-' + self.polarization.lower() #sk edit grd to grdh
	...
	def parse(self):
		...
		if len(self.xml)>0: #Read from zip file
			try:
				parts = self.xml.split(os.path.sep)
				zipname = os.path.join('/',*(parts[:-3]))
					#old: zipname = os.path.join(*(parts[2:-3]))
	...
	def populateIPFVersion(self):
		...
		if len(self.manifest)>0: #Read from zip file
			import zipfile	  
